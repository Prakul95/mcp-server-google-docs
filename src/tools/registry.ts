...
export const ToolSchemas = {
  ...
  "list-docs": z.object({
    ...
    orderBy: z.string().optional().describe(
      "Drive orderBy string, e.g., 'modifiedTime desc,name'."
    ),
    pageSize: z.number().int().min(1).max(1000).optional().describe(
      "Max items per page (Drive default 100; max 1000)."
    ),
    pageToken: z.string().optional().describe(
      "Drive page token from a previous list."
    ),
    spaces: z.string().optional().describe(
      "Drive spaces to search (e.g., 'drive'). Defaults to 'drive'."
    )
  }),
  ...
  "get-doc": z.object({
    docsArgs: z.object({
      ...
      includeTabsContent: z.boolean().optional().describe(
    ...
    formattingArgs: z.object({
      includePreview: z.boolean().optional().describe(
        "If true, returns preview of document; otherwise returns the full document."),
        previewChars: z.number().int().min(200).optional().describe("Max chars per document."),}).optional(),
      }),
...
"update-doc": z.object({
  ...
  documentId: z.string().min(1).describe("The Google Docs documentId to update."),

  /** Request body metadata (Schema$BatchUpdateDocumentRequest) */
  requestBody: z.object({
    writeControl: z.object({
      requiredRevisionId: z.string().describe(
    ...
    requests: z.array(
      z.object({
        ...
        const provided = keys.filter(k => (req as any)[k] !== undefined);
        if (provided.length !== 1) {
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
    ...
    ).min(1).describe("One or more Docs requests; each object must specify exactly one operation."),
  }),
}),
  ...
  "copy-doc": z.object({
    ...
    name: z.string().optional().describe("Optional new name for the copied Doc."),
    parents: z.array(z.string()).optional().describe(
      "Optional array of Drive folder IDs to place the copy in."
    ),
    keepRevisionForever: z.boolean().optional().describe(
  ...
  "list-tabs": z.object({
    ...
    includeChildTabs: z.boolean().optional().describe(
...
export class ToolRegistry {
  ...
  {
    name: "list-docs",
    description: "List Google Docs from Drive. Can filter by folder ID and additional Drive query parameters.",
    schema: ToolSchemas["list-docs"],
    handler: ListDocsHandler
  },
  {
    name: "get-doc",
    description: "Retrieve a Google Doc's metadata and/or content. Supports fetching tab content.",
    schema: ToolSchemas["get-doc"],
    handler: GetDocHandler
  },
  {
    name: "create-doc",
    description: "Create a new blank Google Doc with a given title. Optionally place it in a Drive folder.",
