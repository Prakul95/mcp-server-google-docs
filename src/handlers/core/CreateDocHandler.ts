...
export class CreateDocHandler extends BaseToolHandler {
    async runTool(args: CreateDocInput , oauth2Client: OAuth2Client): Promise<CallToolResult> {
        const validArgs = args as CreateDocInput;
    ...
    private async createDoc(client: OAuth2Client, args:CreateDocInput ): Promise<docs_v1.Schema$Document> {
            try {
