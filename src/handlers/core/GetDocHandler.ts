...
import { GetDocInput } from "../../tools/registry.js";
import { BaseToolHandler } from "./BaseToolHandler.js";
import { docs_v1 } from "googleapis";

export class GetDocHandler extends BaseToolHandler {
  async runTool(
    args: GetDocInput,
    oauth2Client: OAuth2Client,
  ): Promise<CallToolResult> {
    const validArgs = args;
    const doc = await this.getDoc(oauth2Client, validArgs);
    return {
      content: [
        {
          type: "text", // This MUST be a string literal
          text: this.formatDocsList(doc, validArgs),
        },
      ],
    };
  }
  private async getDoc(
    ...
    try {
      ...
      const response = await doc.documents.get(args.docsArgs);
      return response.data || [];
    } catch (error) {
      throw this.handleGoogleApiError(error);
    }
  }
  private formatDocSummary(
    ...
    const revisionId = this.sanitizeString(
      document.revisionId ?? "unknown-revision",
    );
    ...
    const hasFooters =
      !!document.footers && Object.keys(document.footers).length > 0;

    let line =
      `${title} (${documentId})  rev:${revisionId}  ` +
    ...
    if (includePreview) {
      ...
      const fullText = this.extractTextPreview(
        document,
  ...
  private countDocElements(document: docs_v1.Schema$Document): {
    paragraphs: number;
    tables: number;
    images: number;
  } {
   ...
   */
  private formatDocsList(
    document: docs_v1.Schema$Document,
    opts?: GetDocInput,
  ): string {
    if (!document) return "No documents found";
    return this.formatDocSummary(document, opts?.formattingArgs);
  }
  /**
